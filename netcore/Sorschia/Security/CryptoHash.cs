...
namespace Sorschia.Security
{
    public sealed class CryptoHash
    {
        ...
        private byte[] ComputeHash<THashAlgorithm>(Func<THashAlgorithm> createAlgorithm, byte[] input) where THashAlgorithm : HashAlgorithm
        {
        ...
        private string ComputeHash<THashAlgorithm>(Func<THashAlgorithm> createAlgorithm, string input) where THashAlgorithm : HashAlgorithm => ToString(ComputeHash(createAlgorithm, _utf8.GetBytes(input)));
